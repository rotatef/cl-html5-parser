...
(deftest test-reappend-child ()
  (let* ((parent1 (make-element "parent1" nil))
         (parent2 (make-element "parent2" nil))
         (child (make-element "child" nil)))
...
(deftest test-navigate ()
  (let* ((parent (make-element "parent" nil))
         (child1 (make-element "child1" nil))
         (child2 (make-element "child2" nil))
         (child3 (make-element "child3" nil))
         (child4 (make-element "child4" nil)))
...
(deftest test-set-attribute ()
  (let* ((element (make-element "test" nil)))
    (setf (element-attribute element "hello") "world")
    (is (string= (element-attribute element "hello") "world"))))

(deftest test-append-text ()
  (let ((parent (make-element "parent" nil)))
    (html5-parser::node-append-child* parent (make-text-node "hello"))
    (html5-parser::node-append-child* parent (make-text-node "world"))
